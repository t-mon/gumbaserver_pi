    ...
    QObject(parent)
{
    ...
    //    connect(m_server,SIGNAL(movementTurnRight()),m_robot,SLOT(movementTurnRight()));
        ...
        connect(m_server,SIGNAL(movementCommand(QString)),m_robot,SLOT(movementCommand(QString)));
    ...
    connect(m_server,SIGNAL(servoChanged(int,QString)),m_servo,SLOT(setServo(int,QString)));
